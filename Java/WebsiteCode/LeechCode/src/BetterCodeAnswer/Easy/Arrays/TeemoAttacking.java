...
class TeemoAttacking_Solution {
    public int findPoisonedDuration(int[] timeSeries, int duration) {
        ...
        int i;
        int total = duration;
        int previousAttack = timeSeries[0] + duration - 1;
        for (i = 1; i < length; i++) {
            if (timeSeries[i] > previousAttack) {
                total = total + duration;
            } else {
                total = total + timeSeries[i] + duration - previousAttack - 1;
            }
            previousAttack = timeSeries[i] + duration - 1;
        }
        return total;
    }
}