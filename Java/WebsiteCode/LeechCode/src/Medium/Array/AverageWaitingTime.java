...
/**
 * <a class="no-underline hover:text-blue-s dark:hover:text-dark-blue-s truncate cursor-text whitespace-normal hover:!text-[inherit]" href="/problems/average-waiting-time/">1701. Average Waiting Time</a>
 * 
 * <div class="elfjS" data-track-load="description_content"><p>There is a restaurant with a single chef. You are given an array <code>customers</code>, where <code>customers[i] = [arrival<sub>i</sub>, time<sub>i</sub>]:</code></p>
 * 
 * <ul>
...
public class AverageWaitingTime {
    public static void main(String[] args) {
        ...
        for (int[][] customers : tests) {
            System.out.println(new AverageWaitingTime_Solution().averageWaitingTime(customers));
        }
    }
}

// 3 ms 72.4 MB
class AverageWaitingTime_Solution {
    public double averageWaitingTime(int[][] customers) {
        ...
        for (int[] customer : customers) {
            int arriveTime = customer[0],
                prepTime = customer[1];
            now = arriveTime > now ? arriveTime : now;
            
            timeDone = now + prepTime;
            totalPrepTime += (double) (timeDone - arriveTime)/customers.length; 
            now = timeDone;
        }
