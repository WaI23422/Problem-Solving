...
// @see BetterCodeAnswe.Hard
class RobotCollisions_Solution {
    public List<Integer> survivedRobotsHealths(int[] positions, int[] healths, String directions) {
        int len = positions.length;
        
        HashMap<Integer,int[]> robotLeft = new HashMap<>(),
                                 robotRight = new HashMap<>();

        for (int i = 0; i < len; i++) {
            if (directions.charAt(i) == 'R') {
                robotLeft.put(i, new int[]{positions[i],healths[i]});
            } else {
                robotRight.put(i, new int[]{positions[i],healths[i]});
            }
        }   
        
        if (robotLeft.size() == 0 || robotRight.size() == 0) {
